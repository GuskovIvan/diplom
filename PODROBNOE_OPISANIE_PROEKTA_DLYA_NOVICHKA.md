...
- `OWNER` - владелец проекта, может назначать администраторов и управлять всеми участниками;
- `ADMIN` - администратор, может управлять колонками, задачами и обычными участниками;
...
`ownerId` связывает проект с владельцем.

`version` нужна для общей версии проекта.

Связи:

- проект имеет владельца;
